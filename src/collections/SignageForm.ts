...
export const SignageForm: CollectionConfig = {
  slug: 'signage-forms',
  admin: {
    useAsTitle: 'fullName',
      ...
      async ({ doc, operation, req }) => {
        ...
        if (adminEmail) {
          sendBrevo({
            ...
            subject: `📩 New Signage Enquiry — ${doc.fullName}`,
            htmlContent: `
              <h2 style="color:#333;font-family:sans-serif">New Signage Form Submission</h2>
        ...
        try {
          const { docs: webhooks } = await req.payload.find({
            ...
            where: {
              and: [
                { enabled: { equals: true } },
                { triggerOn: { equals: 'signageFormSubmission' } },
              ],
            },
            limit: 20,
          })
