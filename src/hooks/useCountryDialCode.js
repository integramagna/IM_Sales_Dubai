...
const DIAL_CODES = {
  ...
  QA: '+974', RO: '+40', RU: '+7', RW: '+250', KN: '+1', LC: '+1',
  ...
  TT: '+1', TN: '+216', TR: '+90', TM: '+993', TV: '+688', UG: '+256',
  UA: '+380', AE: '+971', GB: '+44', US: '+1', UY: '+598', UZ: '+998',
  VU: '+678', VE: '+58', VN: '+84', YE: '+967', ZM: '+260', ZW: '+263',
}

export function useCountryDialCode() {
  const [dialCode, setDialCode] = useState('')

  useEffect(() => {
    const cached = sessionStorage.getItem('im_dial_code')
    if (cached) {
      setDialCode(cached)
      return
    }
    ...
    fetch('https://ipapi.co/json/', { signal: controller.signal })
      .then((res) => res.json())
      .then((data) => {
        const code = DIAL_CODES[data.country_code]
        if (code) {
          const value = code + ' '
          sessionStorage.setItem('im_dial_code', value)
          setDialCode(value)
        }
      })
      .catch(() => {})
